...
const BUG_SECTIONS: Section[] = [
    {
        ...
        question: "Summary",
        context:
            "Concise description (e.g., [Component] - [Error] when [Action])",
    },
    {
        id: "environment",
        question: "Environment",
        context: "OS, Browser, Version, Environment (Prod/Staging)",
    },
    {
        id: "description",
        question: "Description",
        context: "High-level overview of the issue",
    },
    {
        id: "steps",
        question: "Steps to Reproduce",
        context: "Numbered list of steps",
    },
    {
        id: "expected",
    ...
    },
    {
        ...
        context: "Code references, potential root cause",
    },
    {
        id: "visuals",
        question: "Visuals/Logs",
        context: "Placeholders for screenshots, stack traces",
    },
    {
        id: "priority",
        question: "Severity/Priority",
        context: "Suggested level based on impact",
    },
];
...
const TICKET_SECTIONS: Section[] = [
    {
        ...
        context: "Clear, concise, prefixed with component",
    },
    {
        id: "user_story",
        question: "User Story",
        context: "As a [role], I want to [action], so that [value]",
    },
    {
        id: "context",
    ...
    {
        id: "ac",
        question: "Acceptance Criteria (AC)",
        context: "Checklist format (Gherkin preferred)",
    },
    {
        id: "dod",
        question: "Definition of Done (DoD)",
        context: "Standard engineering requirements",
    },
    {
        id: "security",
...
class WizardComponent implements Component {
    ...
    constructor(
        sections: Section[],
        tui: TUI,
        onDone: (result: Record<string, string> | null) => void,
    ) {
    ...
    render(width: number): string[] {
        ...
        for (let i = 0; i < this.sections.length; i++) {
            ...
            else if (this.answers[i].trim().length > 0)
        ...
        if (this.showingConfirmation) {
            ...
            lines.push(
                padToWidth(
                    boxLine(
                        `${this.yellow("Create file?")} ${this.dim("(Enter/y to confirm, Esc/n to cancel)")}`,
                    ),
                ),
            );
        } else {
            ...
            lines.push(
                padToWidth(
                    boxLine(
                        truncateToWidth(
                            `${this.dim("Tab")} next · ${this.dim("Shift+Tab")} prev · ${this.dim("Enter")} confirm · ${this.dim("Esc")} cancel`,
                            contentWidth,
                        ),
                    ),
                ),
...
export default function(pi: ExtensionAPI) {
    const handleCommand = async (
        ...
        const templateFile =
            type === "bug" ? "golden-bug.md" : "golden-ticket.md";
        const templatePath = path.join(
            "/Users/mike/.dotfiles/nvim/.config/nvim/prompts",
            templateFile,
        );
        ...
        if (args && args.trim().length > 0) {
            // LLM generation
            markdownContent =
                (await ctx.ui.custom<string>((tui, theme, _kb, done) => {
                    const loader = new BorderedLoader(
                        tui,
                        theme,
                        `Generating Golden ${type}...`,
                    );
                    loader.onAbort = () => done("");
                    generateWithLLM(ctx, args, template)
                        .then(done)
                        .catch((err) => {
            ...
            const match =
                markdownContent.match(/#+\s*(?:Summary|Title):?\s*(.*)/i) ||
                markdownContent.match(/1\.\s+\*\*Summary\*\*:\s*(.*)/i);
            summary = match
                ? match[1].trim()
                : args.split(" ").slice(0, 2).join(" ");
        } else {
            // Wizard
            const results = await ctx.ui.custom<Record<string, string> | null>(
                (tui, _theme, _kb, done) => {
            ...
            const wizardPrompt =
                `Please expand on these notes for a ${type === "bug" ? "bug report" : "ticket"} and provide a high-quality, professional document:\n\n` +
            ...
            markdownContent =
                (await ctx.ui.custom<string>((tui, theme, _kb, done) => {
                    const loader = new BorderedLoader(
                        tui,
                        theme,
                        `Expanding Golden ${type}...`,
                    );
                    loader.onAbort = () => done("");
                    generateWithLLM(ctx, wizardPrompt, template)
                        .then((content) => {
                            // Strip anything before the first markdown header or horizontal rule if present
                            const lines = content.split("\n");
                            let firstHeaderIndex = lines.findIndex(
                            ...
                            if (lines[firstHeaderIndex].startsWith("---")) {
                                ...
                                if (nextIndex !== -1) {
                                    const postFrontMatter = lines.slice(
                                        firstHeaderIndex + nextIndex + 2,
                                    );
                                    const actualContentIndex =
                                        postFrontMatter.findIndex((l) =>
                                            l.trim().startsWith("#"),
                                        );
                                    if (actualContentIndex !== -1) {
                                        done(
                                            postFrontMatter
                                                .slice(actualContentIndex)
                                                .join("\n")
                                                .trim(),
                                        );
                                        return;
